import { integer, pgEnum, pgTable, text, timestamp, varchar, jsonb, index, unique, boolean, numeric } from "drizzle-orm/pg-core";




export const roleEnum = pgEnum("role", ["owner", "collaborator", "restricted", "admin", "user"]);
export const fitTierEnum = pgEnum("fitTier", ["A", "B", "C"]);
export const intentTierEnum = pgEnum("intentTier", ["Hot", "Warm", "Cold"]);
export const lifecycleStageEnum = pgEnum("lifecycleStage", ["Lead", "MQL", "SQL", "Opportunity", "ClosedWon", "ClosedLost"]);
export const seniorityEnum = pgEnum("seniority", ["C-Level", "VP", "Director", "Manager", "IC", "Other"]);
export const statusEnum = pgEnum("status", ["active", "waiting", "dormant", "closed"]);
export const typeEnum = pgEnum("type", [ "email_sent", "email_received", "reply_received", "call_completed", "meeting_held", "note_added", "signal_detected", "lead_captured", "deal_won", "deal_lost" ]);
export const providerEnum = pgEnum("provider", ["google", "amplemarket", "whatsapp", "apollo"]);
export const eventTypeEnum = pgEnum("eventType", ["sent", "opened", "replied", "bounced", "unsubscribed"]);
export const nodeTypeEnum = pgEnum("nodeType", ["email", "wait", "condition", "ab_split", "goal_check", "exit"]);
export const conditionTypeEnum = pgEnum("conditionType", [ "replied", "not_replied", "opened", "not_opened", "clicked_link", "time_elapsed", "custom_field", "goal_achieved", "negative_response" ]);
export const goalTypeEnum = pgEnum("goalType", ["meeting_booked", "demo_requested", "replied", "link_clicked", "custom"]);
export const edgeTypeEnum = pgEnum("edgeType", ["default", "yes", "no", "variant_a", "variant_b", "goal_met", "goal_not_met"]);
export const triggerTypeEnum = pgEnum("triggerType", [ "email_opened", "email_replied", "no_reply_after_days", "meeting_held", "stage_entered", "deal_value_threshold", "scheduled" ]);
export const actionTypeEnum = pgEnum("actionType", [ "move_stage", "send_notification", "create_task", "enroll_sequence", "update_field" ]);
export const categoryEnum = pgEnum("category", ["lead_nurturing", "deal_management", "task_automation", "notifications"]);
export const priorityEnum = pgEnum("priority", ["low", "medium", "high", "urgent"]);
export const linkedEntityTypeEnum = pgEnum("linkedEntityType", ["deal", "contact", "account"]);
export const entityTypeEnum = pgEnum("entityType", ["deal", "contact", "account", "task", "email"]);
export const viewTypeEnum = pgEnum("viewType", ["deals", "contacts", "accounts", "tasks"]);
export const sortOrderEnum = pgEnum("sortOrder", ["asc", "desc"]);
export const activityTypeEnum = pgEnum("activityType", ["email", "call", "meeting", "note", "task", "deal_stage_change", "tag_added", "assignment_changed"]);
export const syncTypeEnum = pgEnum("syncType", ["full", "incremental", "preview", "list_counts"]);
...
export const accounts = pgTable("accounts", {
  ...
  enrichmentSnapshot: jsonb("enrichmentSnapshot").$type<Record<string, any>>(),
  ...
  fitTier: fitTierEnum(),
  intentTier: intentTierEnum(),
  scoreReasons: jsonb("scoreReasons").$type<string[]>(),
...
export const people = pgTable("people", {
  ...
  secondaryEmails: jsonb("secondaryEmails").$type<string[]>().default([]),
...
export const threads = pgTable("threads", {
  ...
  collaboratorUserIds: jsonb("collaboratorUserIds").$type<number[]>(),
  visibility: varchar("visibility", { length: 20 }).default("private"),
  dealSignal: jsonb("dealSignal").$type<{value_estimate?: number; confidence?: 'low'|'medium'|'high'; outcome?: 'won'|'lost'}>(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({
...
export const events = pgTable("events", {
  ...
  formSchema: jsonb("formSchema").$type<{
    fields: Array<{
      key: string;
      label: string;
      type: string;
  ...
  defaultTags: jsonb("defaultTags").$type<string[]>().default([]),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({
...
export const integrations = pgTable("integrations", {
  ...
  status: statusEnum().default("disconnected").notNull(),
  config: jsonb("config").$type<Record<string, any>>().default({}),
  oauthTokens: jsonb("oauthTokens").$type<Record<string, any>>(),
...
export const sequenceNodes = pgTable("sequenceNodes", {
  id: varchar("id", { length: 36 }).primaryKey(),
  sequenceId: varchar("sequenceId", { length: 36 }).notNull(),
  nodeType: nodeTypeEnum().notNull(),
  position: jsonb("position").$type<{ x: number; y: number }>().notNull(), // For visual builder
  ...
  conditionConfig: jsonb("conditionConfig").$type<Record<string, any>>(), // e.g., {field: "jobTitle", operator: "equals", value: "CEO"}
...
export const sequenceEdges = pgTable("sequenceEdges", {
  ...
  edgeType: edgeTypeEnum().notNull().default("default"),
...
export const enrollmentPathHistory = pgTable("enrollmentPathHistory", {
  ...
  metadata: jsonb("metadata").$type<Record<string, any>>().default({}), // e.g., {variant: "A", conditionMet: true}
}, (table) => ({
...
export const automationRules = pgTable("automationRules", {
  ...
  triggerConfig: jsonb("triggerConfig").$type<Record<string, any>>().default({}),
  actionType: actionTypeEnum().notNull(),
  actionConfig: jsonb("actionConfig").$type<Record<string, any>>().default({}),
...
export const trackingEvents = pgTable("trackingEvents", {
  ...
  accountId: varchar("accountId", { length: 36 }),
  eventType: eventTypeEnum().notNull(),
  eventData: jsonb("eventData").$type<Record<string, any>>().default({}),
  timestamp: timestamp("timestamp").defaultNow().notNull(),
}, (table) => ({
  tenantPersonIdx: index("tenant_person_idx").on(table.tenantId, table.personId),
...
export const channels = pgTable("channels", {
  ...
  name: varchar("name", { length: 100 }).notNull(),
  description: text("description"),
  type: typeEnum().default("public").notNull(),
...
export const calendarEvents = pgTable("calendarEvents", {
  ...
  status: statusEnum().default("confirmed").notNull(),
...
export const emailExamples = pgTable("emailExamples", {
  ...
  performanceMetrics: jsonb("performanceMetrics"), // e.g., { openRate: 0.8, replyRate: 0.3 }
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({
...
export const amplemarketSyncLogs = pgTable("amplemarketSyncLogs", {
  ...
  metadata: jsonb("metadata").$type<Record<string, any>>(), // Additional sync details
...
export const leads = pgTable("leads", {
  ...
  sequenceIds: jsonb("sequenceIds").$type<string[]>(),
